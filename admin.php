    ...
    <div class="container-fluid">
      ...
      <?php require("_admin_menu.php"); ?>
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
         ...
         <h2 class="sub-header">รายการคงเหลือทั้งหมด</h2>                          
          <div class="table-responsive">
            <table class="table table-striped table-hover">
              <thead>
                <tr class="bg-danger">
					<th>หมวดวัสดุ</th>
					<th>รหัส<br>รายการ</th>
                  <th>รายการ</th>
                  <th>คงเหลือ</th>
                  <th>หน่วยนับ</th>
                  <th>สถานะ</th>
                  <th>จำนวนคงเหลือต่ำสุด<br/>ที่สามารถเบิกได้</th>                  
					<th></th>
                </tr>
              </thead>
              <tbody>
...
order by c.category,c.code";
			...
			if ( $result->num_rows > 0 ) {
				...
				while ( $row = $result->fetch_assoc() ) {
					...
					echo "<td>".$row["number_low"]."&nbsp;(";
					if($row[ "balance" ]>$row["number_low"]){
						echo "คงเหลือให้เบิกได้";
					}else{
						echo "คงเหลือ<b>ไม่ให้เบิก</b>";
					}					
					echo ")</td>";
					echo '<td><a target="_top" href="admin_edit_register.php?menu=1&id=' . $row[ "id" ] . '" class="btn btn-info" role="button">แก้ไขข้อมูล</a></td>';
					echo "</tr>";
				}	
			} else {
				//not found
