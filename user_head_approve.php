...
if ( isset( $_POST[ "check" ] ) ) {
	...
	foreach ( $number as $key => $value ) {
		$sql = "UPDATE request_approve SET req_number=$number[$key], head_app=1 where id=$key";
	...
	$conn->close();
	if ( $showStatus ) {
		header( "Location: ./user_head_check_approve.php?menu=3&chk=ok" );
		exit;
		return;
	}
}
...
<body>
	...
	<div class="container-fluid">
		<div class="row">
			<?php require("_user_menu.php"); require_once( "_my_function.php" );?>
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<div class="table-responsive">
					<form action="?menu=3&d=<?php echo $_GET['d']; ?>&name=<?php echo $_GET['name']; ?>&uid=<?php echo $_GET['uid']; ?>" method="post" onsubmit="return confirm('ยืนยันจำนวนการขอเบิกดังกล่าวใช่หรือไม่?');">
...
where z.app_number is null and z.req_id_user=" . $_GET[ "uid" ] . " and date(z.req_date)='" . $_GET[ "d" ] . "'
order by c.id,a.code asc";
										...
										if ( $result->num_rows > 0 ) {
											...
											while ( $row = $result->fetch_assoc() ) {
												...
												echo "<td>" . $row[ "unit" ] . "</td>";
												echo '<td><input type="text" name="number_' . $row[ "pk" ] . '" class="form-control col-xs-2" placeholder="จำนวนจ่าย" value="'. $row[ "req" ] .'" id="number_' . $row[ "pk" ] . '"></td>';
												echo "</tr>";
											}
										} else {
											//not found
						...
						<div class="form-group">
							...
							<div class="col-sm-offset-2 col-sm-10">
								<button type="submit" class="btn btn-primary">บันทึกข้อมูลการอนุมัติเบิก</button>
							</div>
						</div>
					</form>
				</div>
